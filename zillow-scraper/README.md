...
[zillow.com](https://zillow.com/) scraper powered by [Scrapeless](https://www.scrapeless.com/). Every surface drives a Scrapeless cloud [Scraping Browser](https://www.scrapeless.com/en/scraping-browser) and emits identical JSON shapes — see [`DATA_MODEL.md`](DATA_MODEL.md).
...
The CLI surface covers `property`; `search` comes from Zillow's `/async-create-search-page-state` endpoint and is available on the `nodejs/` and `python/` surfaces.
...
- [`browser/nodejs/results/property.json`](browser/nodejs/results/property.json)
