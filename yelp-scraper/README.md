...
[yelp.com](https://yelp.com/) scraper powered by [Scrapeless](https://www.scrapeless.com/). Every surface drives a Scrapeless cloud [Scraping Browser](https://www.scrapeless.com/en/scraping-browser) and emits identical JSON shapes — see [`DATA_MODEL.md`](DATA_MODEL.md).
...
The CLI surface covers `business_pages` and `search`; `reviews` comes from Yelp's GraphQL endpoint and is available on the `nodejs/` and `python/` surfaces only.
...
- [`browser/nodejs/results/business_pages.json`](browser/nodejs/results/business_pages.json)
