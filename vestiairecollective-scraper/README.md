...
[vestiairecollective.com](https://vestiairecollective.com/) scraper powered by [Scrapeless](https://www.scrapeless.com/). Every surface drives a Scrapeless cloud [Scraping Browser](https://www.scrapeless.com/en/scraping-browser) and emits identical JSON shapes — see [`DATA_MODEL.md`](DATA_MODEL.md).
...
> The CLI surface covers `products` only — `scrape_search` depends on intercepting the `/v1/product/search` XHR, which the in-page `eval` flow can't drive. Use the Python, Node.js, or MCP surface for search. See [`browser/cli/README.md`](browser/cli/README.md).

## Functions

| Python | Node.js |
| --- | --- |
| `find_hidden_data` | `findHiddenData` |
...
- [`browser/nodejs/results/products.json`](browser/nodejs/results/products.json)
